...
import org.apache.servicecomb.pack.demo.car.Car;
import org.apache.servicecomb.pack.demo.car.CarOrderGrpc;
...
public class BookingController {
  ...
  @GrpcClient(value = "car-grpc-server",interceptors= {GrpcClientInterceptor.class})
  private Channel serverChannel;

  public Car.CarReply orderCarByGrpc(String name,  Integer cars) {
	//  serverChannel=  channelBuilder.intercept(new ClientInterruptImpl()).build();
      CarOrderGrpc.CarOrderBlockingStub stub= CarOrderGrpc.newBlockingStub(serverChannel);     
      Car.CarReply response = stub.order(Car.CarRequest.newBuilder().setName(name).setCars(cars).build());
      return response;
  }
...
//        null, String.class, name, cars);
    ...
    this.orderCarByGrpc(name, cars);
    postCarBooking();
    System.out.println(name + " booking " + rooms + " rooms and " + cars + " cars OK");
